...
import jdbcProject.dao.*;
import jdbcProject.vo.*;
...
public class AptController {
	...
	public void updateMyinfo() {
		...
		if (mvo != null) {
			...
			// 새로운 객체에 수정된 내용 저장
			MemberVO member = new MemberVO(userId, name, addr, mobile);
			ApplicationVO app = new ApplicationVO(userId, name);
	...
	public void updateFamilyInfo() {
		...
		System.out.println("▶ 가족의 이름을 입력해주세요 "); // 회원 1명당 여러 명의 가족이 존재할 수 있으므로
		String famName = sc.next();
		FamilyVO family = dao.getMember(id, famName);

		if (mvo != null && id.equals(family.getMemId()) && famName.equals(family.getFamName())) {
	...
	public void updatePartnerInfo() {
		...
		String id = mvo.getMemId();
			...
			if (mvo != null && id.equals(partner.getMemId())) {
	...
	public void updateSubInfo() {
		...
		if (mvo != null && id.equals(sub.getMemId())) {
			String subNum = sub.getSubNum();
			String date = checkSubdate(); // 날짜 확인 메서드 (청약통장 개설일)
			int deposit = checkDeposit(); // deposit 확인 메서드

			// 납입횟수를 계산하여 자동으로 수정해준다.
			LocalDate today = LocalDate.now();
			LocalDate startDate = LocalDate.parse(date);
			Period period = Period.between(startDate, today); // 두 날짜의 차이

			int year = period.getYears() * 12;
			int months = period.getMonths();
			int count = year + months; // 납입횟수 = 개월 수

			sub = new SubscriptionVO(subNum, id, date, deposit, count);
			dao.updateSubInfo(sub);

			System.out.println("청약통장 정보는 개인정보 확인 후 수정됩니다.");
			System.out.println("********** 수정신청 완료! **********");
			System.out.println();
	...
	public String checkSubdate() {
		while (true) {
			...
			boolean checkData = Pattern.matches("^([12]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01]))$", subdate);
			if (checkData==false || subdate.length() > 10 || subdate.length() < 10 || subdate.charAt(4) != '-'
					|| subdate.charAt(7) != '-') {
	...
	public void showAllApt() {
		...
		while (itr.hasNext()) {
			System.out.println(itr.next()); // 객체 출력
			System.out.println();
			System.out.println();
		}
	}
	...
	public void apply() {
		...
		if (mvo != null) { // 그 회원의 id 가져와서 같은지 비교
			...
			app = new ApplicationVO(memName, aptId, aptName, mvo.getMemId());
			appDAO.apply(app);
	...
	public String checkMobileNumber() {
		while (true) {
			...
			boolean checkNum = Pattern.matches("^01(?:0|1|[6-9])-(?:\\d{3}|\\d{4})-\\d{4}$", mobile);
			if (checkNum==false || mobile.length() > 13 || mobile.length() < 13 || mobile.charAt(3) != '-' || mobile.charAt(8) != '-') {
				System.out.println("######## 다시 입력해주세요 ########");
				System.out.println("입력 형식은 '010-0000-0000' 입니다. ");
				System.out.println();
			} else {
				return mobile;
			}
	...
	public String checkRegdate() {
		while (true) {
			...
			boolean checkData = Pattern.matches("^([12]\\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\\d|3[01]))$", regdate);
			if (checkData==false || regdate.length() > 10 || regdate.length() < 10 || regdate.charAt(4) != '-'
					|| regdate.charAt(7) != '-') {
...
//		CompanyDAO cao = new CompanyDAO();
	...
	List<ApplicationVO> getMemberList(List<ApplicationVO> list) {// 신청자]무주택자 뽑기
		ApplicationDAO adao = new ApplicationDAO();
		HouseDAO hdao = new HouseDAO();
	...
	public void hasPartner(List<ApplicationVO> list, int num) {// 신청자]신혼부부 
		PartnerDAO pdao = new PartnerDAO();
		ApplicationDAO adao = new ApplicationDAO();
